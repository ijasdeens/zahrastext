$(document).ready(function () {
    ...
    function getshowOffDetails(){
        $.ajax({
            url: base_url + 'Controllerunit/getshowOffDetails',
            method: 'POST',
            success: function (data) {
                let getData = JSON.parse(data); 
                getData.map(d => {
                    $('#admin_name').val(d.admin_name); 
                    $('.profile-name admin_name').html(d._admin_name);
                    $('.admin_name').html(d.admin_name);
                    $('#admin_tel').val(d.admin_mob); 
                   
                    $('#admin_email').val(d.admin_gmail);
                }); 
            },
            error: function (err) {
                console.error('Error found', err);
    ...
    $('.change_status_from_wr').click(function(){

        let warehouse_id = parseInt($(this).attr('warehouse_id'));

           $.ajax({
            url: base_url + 'Controllerunit/change_status_from_wr',
            method: 'POST',
             data:{
         ...
         $.ajax({
            ...
            success: function (data) {
                ...
                if(getData==0){
                    ...
                    getData.map(d => {
                        expeneseAmount+=parseFloat(d.expense_amount);
                        reportscount+=1;
                    });
                }
...
<td>${d.status==1 ? '<span class="badge badge-success">SOLD</span>' : '<span class="badge badge-danger">Returned</span>'}</td>
    ...
    $('#choose_outlets_for_particularsearch').change(function(e){

        let event_value = Number(e.target.value); 

        $.ajax({
            url: base_url + 'Controllerunit/choose_outlets_for_particularsearch',
            method: 'POST',
            data:{
            ...
            success: function (data) {
                let getData = JSON.parse(data); 
                if(data==''){
                    $('#sumofcost_in_outlet').html('Rs.00.00'); 
                    $('#sum_of_price_amount_outlet').html('Rs.00.00'); 
                    $('#total_types_of_products_outlet').html('0');
                    $('#sumofquantity').html('0');

                }
                else { 
                    $('#sumofcost_in_outlet').html('Rs '+getData.sumofcost); 
                    $('#sum_of_price_amount_outlet').html('Rs '+getData.sumofprice); 
                    $('#total_types_of_products_outlet').html(getData.sumofproducts);
                    $('#sumofquantity').html(getData.sumofquantity);
                }
    ...
    function findsmsbalance(){

        $.ajax({
            url: base_url + 'Controllerunit/findsmsbalance',
            method: 'POST',
            success: function (data) {
                let getData = data
                $('#sms_balance_count').html(getData);
            },
            error: function (err) {
                alert(err);
            }
